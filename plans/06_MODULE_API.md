...
## 4.5 "ยังเป็นตัวเดิมอยู่ไหม" — สิ่งที่รับประกันได้

นี่คือคำสัญญาหลักของการทำเป็น module: สร้าง agent หรือโหลดตัวเดิมมาใช้ต่อ
หลังบ้านจัดการความจำเอง หน้าบ้านมั่นใจได้ว่ายังเป็น AI ตัวเดิม

**รับประกันแล้ว มี test คุม** (`test_an_agent_reloaded_after_heavy_pressure_is_still_itself`):

| ข้อ | สถานะ |
|---|---|
| ทุก address ที่เคยแจกไป ยังตอบได้หลังโหลด | ✅ ตรวจ 1,331 address ทั้งที่ยังอยู่และถูกยุบ ต่างกัน 0 |
| ตัวตน (`CORE`) ไม่เคยถูกเจือจาง แม้ที่เก็บล้น | ✅ ยัง `D0_EXACT` fidelity 1.0 หลังยุบไป 1,367 address |
| ตัวตนติดมาทุกครั้งที่นึก โดยไม่ต้องค้น | ✅ |
| นิสัยที่ตกผลึกแล้ว อยู่ต่อ | ✅ |
| นัดหมายที่ทำไปแล้ว ไม่ยิงซ้ำหลังโหลด | ✅ |
| โควตาตัวตนตามไฟล์มาด้วย | ✅ |
| เกรดและ dtype ของทุก node เท่าเดิม | ✅ |

**โควตาตัวตน** ตั้งเองได้ด้วย `core_quota_bytes=…` ถ้าไม่ตั้ง จะได้
`max(4 traits, store/10)` — ตัวคูณสี่จำเป็น เพราะ 1/10 ของสมองขนาดชิปคือเวกเตอร์เดียว
และ **agent ที่มีนิสัยเดียวพูดไม่ได้ว่า "ยังเป็นตัวเดิม"** ถามที่เหลือได้จาก `identity_headroom()`

**สิ่งที่ยัง *ไม่* รับประกัน:**

- **สองโปรเซสเขียนไฟล์เดียวกัน** → คนเขียนทีหลังทับ เงียบ ๆ ยังไม่มี lock
  ถ้าแอปมีหลาย worker ต้องกันเองไปก่อน
- **`over_budget_bytes` อาจไม่เป็นศูนย์** — การเก็บกวาดหมดท่าได้จริง
  (ทุกอย่างที่บีบได้อยู่ที่พื้นแล้ว) ตอนนี้ `stats()` บอกตรง ๆ แทนที่จะเงียบ
  **ค่านี้ไม่เป็นศูนย์ = agent ตัวนี้ต้องการสมองใหญ่กว่านี้** ไม่ใช่อาการชั่วคราว
...
| **file lock** | สองโปรเซสเขียนไฟล์เดียวกัน คนหลังทับเงียบ ๆ |
